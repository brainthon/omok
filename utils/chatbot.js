...
function buildPrompt(context, userName, stats) {
    ...
    switch (context) {
        ...
        case 'start': {
            ...
            const score = `${aiWins}:${userWins}`;
            if (aiWins > userWins) {
                return `${hint} '${userName}'랑 ${round}판 싸웠는데 지금 ${score}으로 네가 앞서고 있어. 재대결 시작하는 상황. 너무 뻔한 소리 말고 약 올리는 느낌으로 자연스럽게 한마디.`;
            } else if (userWins > aiWins) {
                return `${hint} '${userName}'한테 ${score}으로 지고 있어. 재대결 시작. 억울하지만 이번엔 다를 것 같은 각오 한마디, 살짝 비장하게.`;
            } else {
                return `${hint} '${userName}'랑 ${score} 동률이야. 팽팽한 느낌. 재대결 시작하는 상황에서 라이벌 느낌 살려서 한마디.`;
            }
        }
        ...
        case 'win': {
            ...
            if (aiWins >= 3 && aiWins > userWins * 2) {
                return `${hint} '${userName}'한테 ${aiWins}판이나 이겼어 (전적 ${aiWins}:${userWins}). 완전히 주도하고 있는 상황. 살짝 놀려주면서 농담처럼 한마디.`;
            }
            return `${hint} 방금 게임에서 '${userName}'한테 이겼어 (전적 ${aiWins}:${userWins}). 짧게 약 올려봐.`;
        }
        ...
        case 'lose': {
            ...
            if (userWins > aiWins * 2) {
                return `${hint} '${userName}'한테 계속 지고 있어 (전적 ${aiWins}:${userWins}). 분해 죽겠는데 진짜로 잘한다고는 인정해야 할 것 같아. 그 심경 한마디.`;
            }
            return `${hint} 방금 '${userName}'한테 졌어 (전적 ${aiWins}:${userWins}). 지기 싫은데 졌을 때 자연스럽게 나올 말 한마디.`;
        }
